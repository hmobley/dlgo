...
# tag::mcts-rounds[]
        #print('making rounds {}'.format(self.num_rounds))
        for i in range(self.num_rounds):
            node = root
            while (not node.can_add_child()) and (not node.is_terminal()):
                #print('round {}: selecting child'.format(i))
            ...
            #print('round {}: simulating a random game'.format(i))
            winner = self.simulate_random_game(node.game_state)
            #print('round {}: simulation, check!'.format(i))

            # Propagate scores back up the tree.
            #j = 0
            while node is not None:
                #print('up the tree {}'.format(j))
